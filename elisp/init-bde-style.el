...
(defun bde-set-c-style ()
  (c-set-style "bde"))

(add-hook 'c++-mode-hook 'bde-set-c-style)
;;; Enable auto indent
...
(defun bde-aligh-right ()
  ...
  (let ((right-thing-length 0)
    ...
    (setq num-spaces (- bde-max-linelength right-thing-length (current-column)))
    (if (> num-spaces 0)
  ...
  (let ((args (split-string (thing-at-point 'list) ","))
    ...
    (save-excursion
      (let ((start-col (1+ (current-column)))
        ...
        (when (> max-col bde-max-linelength)
          (backward-list)
          (forward-char)
          (newline)
          (backward-char 2)
          ...
          (let ((longest-length (- max-col start-col)))
            (if (<= longest-length bde-max-linelength)
                (indent-region (region-beginning) (region-end)
                               (- bde-max-linelength longest-length))
    ...
    (if (string= "" (pg/string-trim arglist))
      ...
      (let ((args (split-string arglist ","))
        ...
        (save-excursion
          (let ((start-col (1+ (current-column)))
                (max-col (bde-max-column-in-region)))
            (when (> max-col bde-max-linelengh)
              (backward-list)
              (forward-char)
              (newline)
              (backward-char 2)
...

(defun bde-tag-return (&optional arg)
  "Tags the current line as being a return statement."
  (interactive "p")
  (save-excursion
    (move-end-of-line nil)
    (insert " // RETURN")
    (bde-aligh-right)))
(define-key c-mode-base-map (kbd "C-c r") #'bde-tag-return)

